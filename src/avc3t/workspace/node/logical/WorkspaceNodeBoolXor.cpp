#include "WorkspaceNodeBoolXor.h"

namespace AVC3T {
    WorkspaceNodeBoolXor::WorkspaceNodeBoolXor() :
        WorkspaceNode("Xor", {WorkspaceNodeInput(WorkspaceIOType_Bool), WorkspaceNodeInput(WorkspaceIOType_Bool)},
                      {WorkspaceNodeOutput(WorkspaceIOType_Bool, {&m_OutputValue}, -1)}, {IM_COL32(190, 35, 52, 255)}),
        m_OutputValue(false) {}

    WorkspaceNodeBoolXor::WorkspaceNodeBoolXor(const WorkspaceNodeBoolXor& other) :
    ...
    void WorkspaceNodeBoolXor::Evaluate(double ts) {
